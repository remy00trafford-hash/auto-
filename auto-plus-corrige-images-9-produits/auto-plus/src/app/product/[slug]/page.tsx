...
export default async function ProductPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main>
    ...
    <section className="detail"><Link href="/products" className="back">← Retour au catalogue</Link><div className="detail-grid"><div className="detail-image"><img src={p.image} alt={p.name}/></div><div className="detail-copy"><div className="tag">{p.offerLabel}</div><h1>{p.name}</h1><p className="lead">{p.description}</p><div className="why"><h2>À propos du produit</h2><p>{p.description} Cette fiche présente les caractéristiques principales du produit afin de vous permettre de décider rapidement s’il correspond à votre besoin.</p></div><Link className="primary detail-cta" href={p.affiliatePath}>Voir l’offre <span>→</span></Link></div></div></section>
  </main>
}
